...
def parse_coinbase_csv(input_path: str) -> List[NormalizedRow]:
    ...
    with open(input_path, newline="", encoding="utf-8") as f:
        ...
        for r in reader:
            ...
            asset = r["Asset"]
            qty = to_decimal(r["Quantity Transacted"]) if r["Quantity Transacted"] else Decimal("0")
            spot_currency = r["Spot Price Currency"]
            spot_price = to_decimal(r["Spot Price at Transaction"]) if r["Spot Price at Transaction"] else Decimal("0")
            subtotal = r["Subtotal"]
            subtotal_val = to_decimal(subtotal) if subtotal else None
            fees = to_decimal(r["Fees and/or Spread"]) if r["Fees and/or Spread"] else Decimal("0")
            ...
            if op == "Advanced Trade Buy":
                ...
                normalized.append(NormalizedRow(
                    ...
                    emitido_valor_eur="",
                    recibido_moneda=spot_currency,
                    recibido_cantidad=subtotal_val if subtotal_val else Decimal("0"),
                    recibido_valor_eur="",
                    comision_moneda=spot_currency,
                    comision_cantidad=fees,
                    comision_valor_eur="",
